import { Sun, Feather, Moon, Flame, Check, ChevronLeft, ArrowRight } from 'lucide-react';

interface HairScreenProps {
  selectedHair: string;
  otherText?: string;
  onSelectHair: (hair: string) => void;
      ...
      <div>
        <div className="pb-3 pt-1">
          <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-[#f1ca63]">
            <span>STEP 7 OF 8 • HAIR COLOR</span>
            <span>88%</span>
          </div>
          {/* Progress Bar */}
        ...
        <div className="space-y-2.5 my-auto py-2">
          {HAIR_OPTIONS.map((item) => {
            const isSelected = selectedHair.toUpperCase() === item.name;
            ...
            return (
              <button
                key={item.name}
                type="button"
                id={`hair-opt-${item.name.toLowerCase()}`}
                onClick={() => onSelectHair(item.name)}
