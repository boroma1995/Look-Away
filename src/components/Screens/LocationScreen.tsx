...
const PRIMARY_LOCATIONS: { name: string; icon: typeof Home }[] = [
  { name: 'HOME', icon: Home },
  { name: 'WORK', icon: Briefcase },
  { name: 'DRIVING', icon: Car },
  { name: 'SHOPPING', icon: ShoppingBag },
  { name: 'CHURCH', icon: Compass },
  { name: 'ON PHONE', icon: Smartphone },
  { name: 'ON TV', icon: Tv },
  { name: 'ON COMPUTER', icon: Monitor },
      ...
      <div>
        <div className="pb-3 pt-1">
          <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-[#f1ca63]">
            <span>STEP 3 OF 8 • LOCATION</span>
            <span>38%</span>
          </div>
          {/* Progress Bar */}
          ...
          {PRIMARY_LOCATIONS.map((item) => {
            ...
            return (
              <button
                key={item.name}
                type="button"
                id={`location-opt-${item.name.toLowerCase().replace(/\s+/g, '-')}`}
                onClick={() => onToggleLocation(item.name)}
